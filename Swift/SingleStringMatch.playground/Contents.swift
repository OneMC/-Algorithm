...
/*
 4 KMP算法:
 KMP算法与BM算法思想类似，通过对模式串的处理，当发生不匹配时，通过查表来计算向后移动的距离。
 但是KMP移动距离只有前缀匹配，所以通常来说移动距离要小于BM算法。这也是KMP效率低于BM算法的原因。
         0123456789
 主串:    ababababca
 初始位置: abababca
 移动:      abababca
 当在6位置出现不匹配字符时，已匹配的前缀是ababab。需要找到ababab最长的前缀与后缀相同的子串，即abab。
 ...
 ababa      babab
 */
func kmpMacth(main:String, pattern:String) -> [Int]{
    var position = [Int]()
    let next = getNext(pattern: pattern)
    var count = 0
    for i in 0..<main.count {
        // 发生不匹配字符时，找到好后缀的下一个字符
        while count > 0 && pattern[pattern.index(pattern.startIndex, offsetBy: count)] != main[main.index(main.startIndex, offsetBy: i)] {
            count = next[count-1]
        }
        
        if pattern[pattern.index(pattern.startIndex, offsetBy: count)] == main[main.index(main.startIndex, offsetBy: i)] {
            count += 1
        }
        if count == pattern.count {
            position.append(i - pattern.count + 1)
            count = next[count - 1]
        }
    }
    return position;
...
func getNext(pattern:String) -> [Int] {
    var maxMatchLengths = Array.init(repeating: 0, count: pattern.count)
    var maxLength = 0
    for i in 1..<pattern.count {
        while maxLength > 0 && pattern[pattern.index(pattern.startIndex, offsetBy: maxLength)] != pattern[pattern.index(pattern.startIndex, offsetBy: i)] {
            maxLength = maxMatchLengths[maxLength - 1]
        }
        if pattern[pattern.index(pattern.startIndex, offsetBy: i)] == pattern[pattern.index(pattern.startIndex, offsetBy: maxLength)] {
            maxLength += 1
        }
...
print("KMP : ", kmpMacth(main: "cabcabcacab", pattern: "cabcac"))
//print("KMP : ", kmpMacth(main: "xacssccabcfdsf234dsf234abcabcabcxyz", pattern: "cabcab"))
